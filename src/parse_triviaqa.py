...
if not pathlib.Path(f'{config.data_dir}/trivia_qa_{model_name}').exists():
    ...
    val_data.save_to_disk(f'{config.data_dir}/trivia_qa_{model_name}')
else:

    val_data = datasets.load_from_disk(f'{config.data_dir}/trivia_qa_{model_name}')
